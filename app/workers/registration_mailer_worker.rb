class RegistrationMailerWorker
  ...
  def self.perform(dfe_id, to_email, subject, message, event_details_options)
    ...
    if (to_email == 'ALL')
      roles = UserRolesUser.includes(:user).joins(:user_role).where('user_roles.name = ? and kommunity_id', NameValues::UserRoleType::MEMBER, dfe.entity.event.kommunity_id)

      roles.each do |role|
        EventCommunicationMailer.registration_email(dfe, subject, message, event_details_options, role.user).deliver_now
      end
